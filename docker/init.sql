...
comment
on column copilot.copilot_id is '自增数字ID';
comment
on column copilot.stage_name is '关卡名';
comment
on column copilot.uploader_id is '上传者id';
comment
on column copilot.views is '查看次数';
comment
on column copilot.rating_level is '评级';
comment
on column copilot.rating_ratio is '评级比率 十分之一代表半星';
comment
on column copilot.hot_score is '热度';
comment
on column copilot.title is '指定干员@Cascade(["copilot_id"], ["copilot_id"])var opers: List<OperatorEntity>?,文档字段，用于搜索，提取到Copilot类型上';
comment
on column copilot.first_upload_time is '首次上传时间';
comment
on column copilot.upload_time is '更新时间';
comment
on column copilot.content is '原始数据';
comment
on column copilot.delete is '作业状态，后端默认设置为公开以兼容历史逻辑[plus.maa.backend.service.model.CopilotSetStatus]';
...
comment
on table comments_area is '评论区表';
comment
on column comments_area.copilot_id is '关联的作业ID';
comment
on column comments_area.from_comment_id is '回复的评论ID';
comment
on column comments_area.uploader_id is '评论者用户ID';
comment
on column comments_area.message is '评论内容';
comment
on column comments_area.like_count is '点赞数';
comment
on column comments_area.dislike_count is '点踩数';
comment
on column comments_area.upload_time is '评论时间';
comment
on column comments_area.topping is '是否置顶';
comment
on column comments_area.delete is '是否删除';
comment
on column comments_area.delete_time is '删除时间';
comment
on column comments_area.main_comment_id is '主评论ID(如果自身为主评论则为null)';
comment
on column comments_area.notification is '邮件通知';
...
comment
on table rating is '评分表';
comment
on column rating.type is '评级类型 (COPILOT/COMMENT)';
comment
on column rating.key is '被评级对象的唯一标识';
comment
on column rating.user_id is '评级的用户ID';
comment
on column rating.rating is '评级 (Like/Dislike/None)';
comment
on column rating.rate_time is '评级时间';
...
comment
on table copilot_set is '作业集表';
comment
on column copilot_set.name is '作业集名称';
comment
on column copilot_set.description is '额外描述';
comment
on column copilot_set.copilot_ids is 'JSON格式存储的作业ID列表';
comment
on column copilot_set.creator_id is '创建者用户ID';
comment
on column copilot_set.create_time is '创建时间';
comment
on column copilot_set.update_time is '更新时间';
comment
on column copilot_set.status is '状态';
comment
on column copilot_set.delete is '是否删除';
...
comment
on table ark_level is '明日方舟关卡表';
comment
on column ark_level.id is '关卡唯一标识';
comment
on column ark_level.level_id is '关卡ID';
comment
on column ark_level.stage_id is '阶段ID';
comment
on column ark_level.sha is 'SHA哈希值';
comment
on column ark_level.cat_one is '分类一';
comment
on column ark_level.cat_two is '分类二';
comment
on column ark_level.cat_three is '分类三';
comment
on column ark_level.name is '关卡名称';
comment
on column ark_level.width is '宽度';
comment
on column ark_level.height is '高度';
comment
on column ark_level.is_open is '是否开放';
comment
on column ark_level.close_time is '关闭时间';
